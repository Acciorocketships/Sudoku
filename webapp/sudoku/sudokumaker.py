...
def sudokuGen():
    ...
    while len(cells) != 0:
        ...
        Lowest = []
        for i in cells:
            lowestNum.append(sudoku[i].lenOfPossible())  ## finds all the lengths of of possible answers for each regenerateing cell
        ...
        if choiceElement.solvedMethod() == False:  ##the actual setting of the cell
            ...
            for i in cells:  ##now we iterate through the regenerateing unset cells and remove the input if it's in the same row, col, or box
...
def solver(sudoku, f = 0):
    ...
    while solvedCells != []:
        for n in solvedCells:
            ...
            for i in cells:  ##now we itterate through the regenerateg unset cells and remove the input if it's in the same row, col, or box
